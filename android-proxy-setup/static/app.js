...
function updateBanner(devices) {
  ...
  if (unique.length === 1) {
    ...
    if (s === 'enabled') {
      ...
      banner.innerHTML = `<span class="emoji pulse">\u{1F6E1}\uFE0F</span><span class="label">Proxy Enabled <span class="detail">${esc(proxyVal)}</span></span>`;
    } else if (s === 'disabled') {
...
function proxyBadge(proxyValue) {
  if (proxyValue === null) {
  ...
  return `<span class="device-badge badge-enabled">${esc(proxyValue)}</span>`;
}
